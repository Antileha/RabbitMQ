...
try:
    ...
    def callback(ch, method, properties, body):
        ...
        print(f'Получен вектор признаков с ID {message_id}: {features}')

        # Предсказание
        pred = regressor.predict(np.array(features).reshape(1, -1))

        # Отправка предсказания в очередь y_pred
        message_y_pred = {
            'id': message_id,
            'body': pred[0]
        }
        ...
        print(f'Предсказание с ID {message_id}: {pred[0]} отправлено в очередь y_pred')
